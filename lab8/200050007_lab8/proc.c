...
{
  int total_pages = 1, pages = 0, pgdir_traverse = 0;
  struct proc *p;
  pte_t *pte;
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
  {
    if (p->pid == pid && p->state != UNUSED)
    {
      ...
      for (int i = 0; i < p->sz; i += 4096)
      {
        ...
        if (*(pte = walkpgdir(p->pgdir, (void *)i, 0)) & PTE_P)
        {
          pgdir_traverse++;
        }
      }
...
int clone(void (*fn)(int *), int *arg, void *stack)
{
  ...
  *myarg = (int)arg;
  ...
  // ------------------------------//
  new_proc->tf->esp = (int)stack + PGSIZE - 2 * sizeof(int *);
  new_proc->tf->ebp = new_proc->tf->esp;
  new_proc->tf->eip = (uint)fn;

  new_proc->tgid = cur_proc->tgid;
  new_proc->tf->eax = 0;

  for (i = 0; i < NOFILE; i++)
    if (cur_proc->ofile[i])
      new_proc->ofile[i] = filedup(cur_proc->ofile[i]);
  new_proc->cwd = idup(cur_proc->cwd);
  safestrcpy(new_proc->name, cur_proc->name, sizeof(cur_proc));
  pid = new_proc->pid;
  acquire(&ptable.lock);
