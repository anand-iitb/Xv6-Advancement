	...
	*/
	...
	for (int i = 0; i < MAX_INODES; i++)
	{
		if (superblock.inode_bitmap[i] == 0)
		{
			...
			write_inode(mount_point, i, &inode);
			return i;
		}
	}
...
void read_inode(int mount_point, int inodenum, struct inode_t *inodeptr)
{
	...
	int block_num;
	if(inodenum>=0 && inodenum<16)
		block_num = 1;
	else if(inodenum>=16 && inodenum<32)
	block_num = 2,inodenum-=16;
	else 
	{
		printf("Error: Inode number out of range\n");
		return;
	}
	char tempBuf[BLOCKSIZE];
	readblock(mounts[mount_point].device_fd, block_num, tempBuf);
	if(mounts[mount_point].fs_number == 1)
	{
		decrypt(mounts[mount_point].key, tempBuf, sizeof(struct metadata_t));
	}
	memcpy(&metadata, tempBuf, sizeof(struct metadata_t));
	memcpy(inodeptr, &(metadata.inodes[inodenum]), sizeof(struct inode_t));
}
...
void write_inode(int mount_point, int inodenum, struct inode_t *inodeptr)
{
	...
	memcpy(&metadata, tempBuf, sizeof(struct metadata_t));
	memcpy(&(metadata.inodes[inodenum]), inodeptr, sizeof(struct inode_t));
	memcpy(tempBuf, &metadata, sizeof(struct metadata_t));
	if(mounts[mount_point].fs_number == 1)
	{
		encrypt(mounts[mount_point].key, tempBuf, sizeof(struct metadata_t));
	}
	writeblock(mounts[mount_point].device_fd, block_num, tempBuf);
}
